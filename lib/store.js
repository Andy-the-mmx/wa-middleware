...
// old Vercel KV product, UPSTASH_REDIS_REST_URL/TOKEN if connected more
// directly, or a store-name-prefixed variant like WA_MMX_KV_REST_API_URL/TOKEN
// when the integration is installed under a custom store name). This checks
// all of them so it works regardless of which one Vercel actually created.
...
const redisUrl =
  ...
  process.env.WA_MMX_KV_REST_API_URL;
const redisToken =
  process.env.KV_REST_API_TOKEN ||
  process.env.UPSTASH_REDIS_REST_TOKEN ||
  process.env.WA_MMX_KV_REST_API_TOKEN;
...
if (!redisUrl || !redisToken) {
  ...
  throw new Error(
    "Redis credentials not found. Expected KV_REST_API_URL/KV_REST_API_TOKEN, " +
      ...
      "WA_MMX_KV_REST_API_URL/WA_MMX_KV_REST_API_TOKEN in the environment. " +
...
// --- Guided lead-capture flow state ------------------------------------------
// Tracks where a PSID is in the name → email → phone flow. Short TTL on
// purpose: if someone abandons mid-flow, their later ordinary messages
// shouldn't be swallowed as answers days later.

const FLOW_KEY_PREFIX = "leadflow:";
const FLOW_TTL_SECONDS = 60 * 60; // 1 hour

async function saveLeadFlowState(psid, state) {
  await redis.set(`${FLOW_KEY_PREFIX}${psid}`, JSON.stringify(state), {
    ex: FLOW_TTL_SECONDS,
  });
}

async function getLeadFlowState(psid) {
  const raw = await redis.get(`${FLOW_KEY_PREFIX}${psid}`);
  if (!raw) return null;
  return typeof raw === "string" ? JSON.parse(raw) : raw;
}

async function deleteLeadFlowState(psid) {
  await redis.del(`${FLOW_KEY_PREFIX}${psid}`);
}
